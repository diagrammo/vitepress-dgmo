...
🔴 **`withDgmo` injects `client.css` itself — do not put the import back in a theme file.** `src/client-css-plugin.ts` is a second Vite plugin, `enforce: 'post'` so the `.md` module is already JavaScript, appending the import rather than prepending it so existing line numbers and sourcemaps hold. It was a hand-written line in the consumer's `.vitepress/theme/index.ts` until 2026-08-26, and a site that did not add it rendered every diagram TWICE, stacked, on a green build (issue 507). `tests/fixture/.vitepress/theme/index.ts` deliberately omits the import — that absence is what proves the injection works, so re-adding it hides the regression. `injectClientCss: false` opts out.
